...
export class GraphComponent {
  ...
  OnInit() {
    ...
    this.graphDataService
      .getGraphData(this.typeLists[this.graphType][this.graphNameIndex].name)
      .subscribe(data => {
  ...
  changeGraphType() {
    // TODO: -  bude volat metodu v service
  ...
  convertCsvStringToJson(csv: string) {
    ...
    return converter.csv2jsonAsync(csv, this.options).then(jsonObjs => {
      const helpArray = Array<LabData>();
      jsonObjs.forEach(element => {
        const dateTimeParts = element.date.split(' ');
        const timeParts = dateTimeParts[1].split(':');
        const dateParts = dateTimeParts[0].split('.');
        // console.log("date: ", timeParts);
        const el = new LabData(
          element.id,
          new Date(
